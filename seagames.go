...
func GoldSearch(C tabCtry, nC, idx int) int {
	...
	for R >= L && hasil == -1 {
		mid := (L+R)/2
		if C[mid].Gmedal > C[idx].Gmedal{
			R = mid - 1
		}else if C[mid].Gmedal < C[idx].Gmedal{
			L = mid + 1
		}else {
			hasil = mid
		}
...
func SilvSearch(C tabCtry, nC, idx int) int {
	...
	for R >= L && hasil == -1 {
		mid := (L+R)/2
		if C[mid].Smedal > C[idx].Smedal{
			R = mid - 1
		}else if C[mid].Smedal < C[idx].Smedal{
			L = mid + 1
		}else {
			hasil = mid
		}
...
func BRSearch(C tabCtry, nC, idx int) int {
	...
	for R >= L && hasil == -1 {
		mid := (L+R)/2
		if C[mid].Bmedal > C[idx].Bmedal{
			R = mid - 1
		}else if C[mid].Bmedal < C[idx].Bmedal{
			L = mid + 1
		}else {
			hasil = mid
		}
